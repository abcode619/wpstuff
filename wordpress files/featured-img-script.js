jQuery(document).ready(function ($) {
    var customMediaSelector = {
      init: function() {
        $('.expat_custom_media_add').on('click', function (e) {
          var $el;
          e.preventDefault();
          $el = customMediaSelector.el = $(this).parent();
          $el.image = $el.find('.expat_custom_media_image');
          $el.id = $el.find('.expat_custom_media_id');
          customMediaSelector.frame().open();
        });
 
        $('.expat_custom_media_remove').on('click', function (e) {
          var $el = $(this).parent();
          e.preventDefault();
 
          $el.find('.expat_custom_media_image').attr('src', '').hide();
          $el.find('.expat_custom_media_id').val('');
          $el.find('.expat_custom_media_add, .expat_custom_media_remove').toggle();
        });
      },
