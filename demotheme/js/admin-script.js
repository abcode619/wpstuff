...
jQuery( document ).ready(function($) { 
    ...
    $( '.demotheme-settings-page .nav-tab-wrapper a:first' ).addClass( 'nav-tab-active' );
    $( '.demotheme-settings-page .demotheme-content div:first' ).show();
    ...
    * Advanced Add Files
    */
    jQuery( document ).on( 'click', '.demotheme-meta-add-fileadvanced', function() {
        var jQueryfirst = jQuery(this).parent().find('.file-input-advanced:last');
        jQueryfirst.clone().insertAfter(jQueryfirst).show();
        jQuery(this).parent().find('.file-input-advanced:last .demotheme-upload-file-link').val('');
        jQuery(this).parent().find('.file-input-advanced:last .demotheme-upload-file-title').val('');
        jQuery(this).parent().find('.file-input-advanced:last .demotheme-upload-file-sub-title').val('');
        jQuery(this).parent().find('.file-input-advanced:last .demotheme-upload-file-url').val('');
        return false;
    });
     ...
     * Advanced Add Files
     */
    jQuery( document ).on( 'click', '.demotheme-delete-fileadvanced', function() {
        ...
        if(count > 1) {
            ...
            alert( DemoThemeAdmin.one_file_min );
        }
        return false;
    });
   
    // WP 3.5+ uploader
    jQuery( document ).on( 'click', '.demotheme-upload-fileadvanced', function(e) {

        e.preventDefault();

        if(typeof wp == "undefined" || DemoThemeAdmin.new_media_ui != '1' ){// check for media uploader
              ...
              window.send_to_editor = function(html) {
                ...
                window.formfield.find('.demotheme-upload-file-link').val(attachmenturl);
                //window.formfield.find('.demotheme-upload-file-name').val(attachmentname);
                tb_remove();
                ...
                file_frame.on( 'insert', function() {
                        ...
                        selection.each( function( attachment, index ) {
                                ...
                                if(index == 0){
                                        ...
                                        window.formfield.find('.demotheme-upload-file-link').val( attachment.url );
                                        //window.formfield.find('.demotheme-upload-file-name').val( attachment.name );
